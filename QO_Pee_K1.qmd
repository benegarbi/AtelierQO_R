...
La fonction `build_dtm` crée le tableau lexicval appellé "Document Term Matrix" dans R.
...
Les lignes du *tableau lexical* correspondent ici aux 2017 réponses extraites de l'enquête ("Population, Espace de Vie, Environnement", Ined 1992) et les colonnes aux 1157 mots distincts du corpus.
...
# Et afficher le nuage !

Ici on chosit d'afficher les mots de plus de 20 occurrences et ne pas afficher les mots outils (fonction `word_cloud`),

```{r nuage}
cloud<-word_cloud(dtm, color="black", n=100, min.freq=20,remove_stopwords=T)
```

Les *fréquences* de quelques mots doivent être précisées dans la légende de ce graphique pour aider la compréhension du graphique.

# Concordances

Les *concordances* permettent, pour chaque mot, de rassembler l'ensemble des textes (ici les questions ouvertes) dans lesquels il apparaît (fonction `concordances`).

Ici par exmple on cherche comprendre pourquoi le mot enquêté apparait dans les réponses.
...
On dispose des variables suivantes : sexe aget matrim pratique enf activite prof dipl vote habitat localite revenu region
...
La lemmatisation (Lebart, Salem) consite à regrouper les mots qui ont une signification proche : les verbes congugués à la forme infinitive, les formes plurielles au singulier, les adjectifs du féminin au masculin. Cette opération permet de réduire la liste de mots distincts et donc la taille du lexique.

Nous allons voir dans cette partie que cette opération peut de faire de différentes façon et nécessite de faire des choix ...
...
On crée un *dictionnaire* associé au lexique avec la fonction `dictionnary`sans les mots-outils et les mots que l'on a choisit de ne pas analyser.

```{r dico, include=FALSE}
...
```{r Export_dic, eval=FALSE, include=FALSE}
